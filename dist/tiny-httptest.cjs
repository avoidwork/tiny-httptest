...
const require$1 = module$1.createRequire((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.src || new URL('tiny-httptest.cjs', document.baseURI).href)));
...
class HTTPTest {
	constructor (uri, method, headers, body, timeout) {
		const parsed = new node_url.URL(uri);
