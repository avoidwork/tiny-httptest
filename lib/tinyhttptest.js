...
class TinyHTTPTest {
	constructor (uri, method, headers, body, timeout, isHttp2) {
		...
		if (parsed.auth) {
			...
			this.options.headers.authorization = `Basic ${btoa(parsed.auth)}`;
		}
	...
	cors (arg) {
		...
		return new Promise((resolve, reject) => {
			...
			if (!this.req) {
				let cmd = this.http2 ? this.http2Request : this.http1Request;
	...
	http2Request () {
		return new Promise((resolve, reject) => {
			...
			this.req.on("response", headers => {
				...
				if (this.options.method === "HEAD" || this.res.statusCode === 204 || this.res.statusCode === 304) {
					resolve();
				}
			});
	...
	process () {
		...
		this.expects.get("values").forEach((v, k) => this.test(v, this.body[k], `Unexpected body value: ${JSON.stringify(v)} !== ${JSON.stringify(this.body[k])}`));
