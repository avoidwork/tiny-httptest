...
class TinyHTTPTest {
	...
	cors (arg) {
		...
		this.expectHeader("access-control-allow-methods", /GET\, HEAD\, OPTIONS/);
		this.expectHeader("access-control-allow-headers", /(, )?content-type(, )?/);
		this.expectHeader("access-control-expose-headers", /(, )?content-type(, )?/);
		...
		return new Promise((resolve, reject) => {
			...
			if (!this.req) {
				cookies = jar.get(this.options.hostname);
	...
	process () {
		...
		if (body) {
			this.test(this.body, body, "Unexpected body value: " + this.body + " !== " + body);
		}
	...
	send (arg) {
		...
		} else if (type === "string" && regex.maybeJsonBody.test(body) && !this.options.headers["content-type"]) {
			this.options.headers["content-type"] = "application/json";
		}
